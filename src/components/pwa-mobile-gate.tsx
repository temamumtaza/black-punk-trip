...
export function PwaMobileGate({ children }: { children: ReactNode }) {
  ...
  return <main className="pwa-gate" aria-busy={access === "checking"}>
    <section className="pwa-gate-card" aria-labelledby="pwa-gate-title">
      ...
      {access === "checking" ? <p className="pwa-gate-status" role="status">Menyiapkan aplikasi…</p> : installedState === "installed" ? <div className="pwa-gate-instructions"><p><strong>Black Punk Trip sudah terpasang.</strong></p><ol><li><Smartphone size={15} aria-hidden="true" /> Kembali ke layar utama perangkat.</li><li>Buka <strong>Black Punk Trip</strong> dari ikon aplikasi untuk melanjutkan.</li></ol><small>Browser tidak diberi izin untuk membuka PWA secara paksa. Halaman ini sengaja meminta kamu melanjutkan lewat aplikasi yang sudah terpasang.</small></div> : iosDevice ? <div className="pwa-gate-instructions"><p>{iosCopy.lead}</p><ol><li><AppleSafariShareIcon /> {iosCopy.tap} <strong>{iosCopy.share}</strong> {iosCopy.inSafari}</li><li><span className="pwa-gate-plus" aria-hidden="true">＋</span> {iosCopy.choose} <strong>{iosCopy.add}</strong>.</li><li><Smartphone size={15} aria-hidden="true" /> {iosCopy.open}</li></ol><small>{iosCopy.fallback}</small></div> : <div className="pwa-gate-instructions"><p>Pasang Black Punk Trip ke perangkatmu, lalu buka dari layar utama.</p>{deferredInstallPrompt ? <button className="btn btn-primary pwa-install-button" type="button" onClick={requestInstall} disabled={isPromptingInstall}><Download size={17} aria-hidden="true" /> {isPromptingInstall ? "Menyiapkan instalasi…" : "Instal Black Punk Trip"}</button> : <ol><li><span className="pwa-gate-menu" aria-hidden="true">⋮</span> Buka menu browser.</li><li>Pilih <strong>Instal aplikasi</strong> atau <strong>Tambahkan ke layar utama</strong>.</li><li><Smartphone size={15} aria-hidden="true" /> Buka Black Punk Trip dari layar utama.</li></ol>}</div>}
