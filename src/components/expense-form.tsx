    ...
    <div className="view-stack form-view">
      ...
      <div className="form-layout">
        <section className="panel expense-form-panel">
          ...
          <div className="two-fields"><Field label="Dibayar oleh" htmlFor="expense-payer"><SelectInput id="expense-payer" value={paidBy} onChange={(event) => setPaidBy(event.target.value)} disabled={locked}>{members.map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</SelectInput></Field><Field label="Tanggal" htmlFor="expense-date" hint="Format dd/mm/aaaa"><TextInput id="expense-date" type="text" inputMode="numeric" value={expenseDate} onChange={(event) => setExpenseDate(formatDateInputValue(event.target.value))} placeholder="dd/mm/aaaa" maxLength={10} autoComplete="off" disabled={locked} /></Field></div>
          <Field label="Kategori"><div className="category-grid">{Object.entries(categoryLabels).map(([value, label]) => <button className={`category-chip ${category === value ? "is-selected" : ""}`} key={value} type="button" aria-pressed={category === value} onClick={() => setCategory(value as ExpenseCategory)} disabled={locked}><span aria-hidden="true">{value === "food" ? "✦" : value === "accommodation" ? "⌂" : value === "transport" ? "↗" : value === "activity" ? "◌" : value === "shopping" ? "□" : "·"}</span>{label}</button>)}</div></Field>
